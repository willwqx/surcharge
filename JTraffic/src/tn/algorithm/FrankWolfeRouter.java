...
public class FrankWolfeRouter implements Router {
	 ...
	 *            Output stream for printing trace information
	 */
	public FrankWolfeRouter(double tolerance, LinkDoublePropertyMap t, PrintStream out) {
			        this.T = t;
	//4.25.2017 thinking about delete this.T
	...
	private void init(Demand demand) {
		...
		//shortestPathRouter = new ShortestPathRouter(network, T, null/* System.out */);// 4/27/2017
		shortestPathRouter = new ShortestPathRouter(network, network.getTravelCost(), null/* System.out */);
	 ...
	 *            flow)
	 */
	private void update(LinkDoublePropertyMap newFlow) {
	 ...
	 */
	private double calculateObjectiveFunction(LinkDoublePropertyMap newFlow) {
		LinkDoublePropertyMap oldFlow = network.resetFlow(newFlow);
	...
	/**
	 * Compute the optimal combination between the current assignment and the
	 * new assignment, by searching for the best combination ratio using the
	 * Golden Section method
	 * 
	 * @param newAssignment
	 *            the new shortest path assignment
	 * @return an assignment that is a linear combination of the current
	 *         assignment and the new assignment
	 */
	private LinkDoublePropertyMap lineSearch(Assignment newAssignment) {
		LinkDoublePropertyMap newFlow = newAssignment.getFlow(); // 1/30/2017 from link probability

		LinkDoublePropertyMap flow_1, flow_2;
		...
		flow_1 = LinkDoublePropertyMap.combine(variable_1, network.getFlow(),
				1 - variable_1, newFlow);
		flow_2 = LinkDoublePropertyMap.combine(variable_2, network.getFlow(),
				1 - variable_2, newFlow);
		...
		do {
			if (printLineSearch && out != null) {
				out.printf("l:(%f,%f), u(%f,%f) - ", variable_1, value_1,
						variable_2, value_2);
			}
			...
			if (value_1 > value_2) {
				...
				variable_2 = lower_limit + ALPHA * (upper_limit - lower_limit);
				flow_2 = LinkDoublePropertyMap.combine(variable_2,
						network.getFlow(), 1 - variable_2, newFlow);
				value_2 = calculateObjectiveFunction(flow_2);
				...
				variable_1 = lower_limit + (1 - ALPHA)
						* (upper_limit - lower_limit);
				flow_1 = LinkDoublePropertyMap.combine(variable_1,
						network.getFlow(), 1 - variable_1, newFlow);
				value_1 = calculateObjectiveFunction(flow_1);
		...
		flow_1 = LinkDoublePropertyMap.combine(result, network.getFlow(),
				1 - result, newFlow);
		currentAssignment = network.combineAssignment(result,
				currentAssignment, 1 - result, newAssignment);
	...
	public Assignment route(Demand demand) {
		...
		LinkDoublePropertyMap newFlow = null, assignmentFlow = null;

		while (error > stop_criterion && iteration < 1000) {
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////	
	public Assignment newroute(Demand demand) {
		network = demand.getNetwork();
	...
	private void newinit(Demand demand) {

		network = demand.getNetwork();

		//shortestPathRouter = new ShortestPathRouter(network, T, null);// 1/30/2017
		shortestPathRouter = new ShortestPathRouter(network, network.getTravelCost(), null/* System.out */);
