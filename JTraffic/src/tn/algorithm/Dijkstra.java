...
import tn.MultipathRoute;
import tn.Network;
import tn.ODPair;
import tn.Route;
import tn.misc.FibonacciHeap;
import tn.misc.FibonacciHeapNode;
...
public class Dijkstra {
	 ...
	 */
	//LinkReadOnlyProperty<Double> linkCosts;
	final LinkReadOnlyProperty<Double> linkCosts;
	 ...
	 */
	public Dijkstra(Network network, LinkReadOnlyProperty<Double> linkCost){
			...
			for (int origin:network.getNodes()){
				...
				if(origin!=destination && shortestRouteCost<Double.POSITIVE_INFINITY){
					for(int link: network.getOutLinks(origin)){
						...
						double linkCost = linkCosts.get(link); //2/1/2017; 7/18/2017
						//double linkCost = linkCosts.get(link) + S.get(link); // 7/18/2017
