...
public abstract class Route {
	 ...
	 * @param flowMap 
	 */
	public void addFlowToFlowMap(double flow, LinkDoublePropertyMap flowMap){
		for(int link: getLinks()) 
			flowMap.add(link, flow * getLinkProbability(link));// 1/30/2017 from link probability * Flow 
			//flowMap.add(link, flow);
	}
	 ...
	 * @return
	 */
	public double getCost(LinkReadOnlyProperty<Double> linkCost){
		double travelCost = 0;
		for(int l: getLinks()){
	...
	private double getMinPropertyValue(LinkReadOnlyProperty<Double> linkProperty){
